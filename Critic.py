...
class Critic:
    ...
    def _build_model(self):
        ...
        critic = Model(inputs=unknown_image, outputs=critic_output).compile(
    ...
    def train(
        ...
        for epoch in range(epochs):
            # Generate fake batch.
            noise = get_noise(
                dist=noise_dist, n_samples=batch_size, length=noise_length
            )
            ...
            target_loss = self.model.train_on_batch(
            ...
            total_loss = compute_total_loss(
                loss_target=target_loss, loss_fake=fake_loss, mean=loss_mean
            )
    ...
    def evaluate(
        self, data_dir, batch_size, rescale_imgs, generator, noise_dist, noise_length
    ):
        """
        ...
        for batch in range(n_batches):
            ...
            if rescale_imgs:
                fake_batch = rescale_pixel_values(fake_batch)
            val_fake_loss = val_fake_loss + self.model.test_on_batch(
                fake_batch, np.ones((batch_size, 1))
            )
